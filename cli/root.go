...
// NewApp assembles the kit application from the dummyjson domain. The
...
//
// To add a command, declare it in dummyjson/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := dummyjson.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(dummyjson.Domain{}).Register(app)
