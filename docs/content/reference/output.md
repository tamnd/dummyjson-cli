...
describes what users get. Pick a format with `-o`, or let dummy choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
dummy <command> -o table   # aligned columns for reading
dummy <command> -o jsonl   # one JSON object per line, for piping
dummy <command> -o json    # a single JSON array
dummy <command> -o csv     # spreadsheet friendly
dummy <command> -o tsv     # tab-separated
dummy <command> -o url     # just the URL column
dummy <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
dummy <command> --fields id,title,url
```
...
```bash
dummy <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
dummy <command>            # a table, because this is a terminal
dummy <command> | wc -l    # JSONL, because this is a pipe
```
